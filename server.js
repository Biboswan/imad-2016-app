...
pool.query('SELECT "Users".username,"Articles".title,"Articles".timestamp FROM "Users","Articles","articles_tag" WHERE IN ($1) AND "articles_tag".article_id="Articles".id AND "Articles".author_id="Users".id ORDER BY "Articles".timestamp DESC',[req.body],function(err,result) {
        if (err) {
              res.status(500).send(err.toString());
           } else {
